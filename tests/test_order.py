...
class TestOrderButton:
    ...
    @allure.title("Проверка кнопок Заказать")
    @pytest.mark.parametrize("order_button", [BaseLocators.ORDER_UPPER, BaseLocators.ORDER_LOWER])
    def test_order_buttons_show_order_page(self, driver, order_button):
        ...
        with allure.step('Проверить адрес страницы заказа'):
            assert order_page.is_order_page(), "Не на странице заказа"    
...
class TestOrderForm:
    ...
    @allure.title('Успешное заполнение формы заказа валидными данными')
    @pytest.mark.parametrize("order_data", [generate_order_data(), generate_order_data()], indirect=True)
    def test_order_form_successful_order(self, order_data):

        order_page = OrderPage(self.driver)

        with allure.step('Начать с первой страницы заполнения формы'):
...
class TestLogos:
    ...
    def test_scooter_logo_show_main_page(self):
    ...
    def test_yandex_logo_redirect_to_dzen(self):
